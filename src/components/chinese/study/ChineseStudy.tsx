...
const HSK_ORDER: Record<HskLevel, number> = {
  1: 1,
  2: 2,
  3: 3,
  4: 4,
...
export function ChineseStudy() {
  ...
  const { learnedChars: hiddenCards, toggleLearned: toggleHiddenCard } =
    useLearnedChars()
  ...
  // Load user edits from Neon on mount
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    return hskFilter === 'all'
      ? allChars
      : allChars.filter((c) => c.hsk === hskFilter)
  }, [allChars, hskFilter])
  ...
  const filteredAll = useMemo(() => {
    ...
    return filtered.filter((c) => {
      const pinyinNorm = c.pinyin
        .toLowerCase()
        .normalize('NFD')
        .replace(/[̀-ͯ]/g, '')
      return (
        chars.includes(c.char) ||
        pinyinNorm.includes(qNorm) ||
  ...
  const visibleGroups = useMemo(() => {
    ...
    return radicalGroups.filter((g) =>
      g.chars.some((c) => hiddenCards.has(c.char))
    )
  ...
  const allVisibleGroups = useMemo(() => {
    ...
    return allRadicalGroups.filter((g) =>
      g.chars.some((c) => hiddenCards.has(c.char))
    )
  ...
  const handleToggleHide = useCallback(
    (char: string) => {
      withAuth(() => toggleHiddenCard(char))
    },
    [withAuth, toggleHiddenCard]
  )

  const handleSave = useCallback(
    (char: string, edits: UserEdits) => {
      withAuth(() => {
        setUserEdits((prev) => {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {filtered.length.toLocaleString()} ký tự · {visibleGroups.length} bộ
          thủ · {hiddenCards.size.toLocaleString()} đã học
        </p>
      </div>
      ...
      {viewMode === 'all' && (
        <div className="flex gap-2 items-center">
          <input
            ...
            onKeyDown={(e) =>
              e.key === 'Enter' && setCommittedSearch(allSearch)
            }
      ...
      {loaded &&
        (viewMode === 'all' ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
            {(showKnownCardsOnly
              ? selectedGroupChars.filter((c) => hiddenCards.has(c.char))
              : selectedGroupChars
            ).map((c) => (
              <CharacterCard
                key={c.char}
      ...
      {loaded && viewMode === 'all' && allVisibleGroups.length === 0 && (
        <p className="text-center text-muted-foreground py-16">
          {committedSearch
            ...
            : showKnownOnly
              ? 'Chưa có từ nào được đánh dấu đã biết.'
